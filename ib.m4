...
AC_ARG_WITH([timestamps],
            [AC_HELP_STRING([--with-timestamps], [Compile with IB CQ timestamps support])],
            [with_ts=yes],
            [with_ts=no])
...
AS_IF([test "x$with_ib" == xyes],
        ...
        AC_CHECK_HEADERS([infiniband/umad.h], [],
                        [AC_MSG_ERROR([ibumad header files not found])])
	AC_CHECK_HEADERS([infiniband/umad_types.h], [],
                        [AC_MSG_WARN([umad_types header file not found])])
        AC_CHECK_LIB([ibumad], [umad_init], [],
            [AC_MSG_ERROR([libibumad not found])])

        AC_CHECK_HEADERS([rdma/rdma_cma.h], [],
                        [AC_MSG_ERROR([RDMA CM header files not found])])
		AC_CHECK_LIB([rdmacm], [rdma_create_event_channel], [],
			[AC_MSG_ERROR([librdmacm not found])])
...
AS_IF([test "x$with_ib" == xyes],
      [
       ...
       AC_CHECK_DECLS([IBV_EXP_DEVICE_ATTR_WITH_TIMESTAMP_MASK,
                       IBV_EXP_DEVICE_ATTR_WITH_HCA_CORE_CLOCK,
                       IBV_EXP_DEVICE_ATTR_EXP_CAP_FLAGS],[],[have_exp_cq_ts=yes],[[#include <infiniband/verbs_exp.h>]])

       AS_IF([test "x$have_exp_cq_ts" != xno -a "x$with_ts" = xyes],
             [AC_DEFINE([HAVE_IB_EXP_CQ_TS], 1, [IB CQ timestamp support])],
             [AC_MSG_WARN([Compiling without cq ts support])])
